...
class Matrices2x2 {
    ...
    buildMatrizDNegative(){
        nerdamer.setVar('D', `matrix([${ (1/this.matriz[0].x) }, ${0}], [${0}, ${ (1/this.matriz[1].y) }])`);
    }

    buildMatrizb(){
        nerdamer.setVar('b', `matrix([${this.matriz[0].k}],[${this.matriz[1].k}])`);
    }
    ...
    buildMatrizL(){
        nerdamer.setVar( 'L', `matrix([0, 0], [ ${-1*(this.matriz[1].x)}, 0 ])` ); 
    }

    buildMatrizU(){
        nerdamer.setVar( 'U', `matrix([0, ${-1*(this.matriz[0].y)}], [ 0, 0 ])` ); 
    }
    ...
    buildResultado(x= 0, y= 0){

        nerdamer.setVar( 'X', `matrix( [${x}], [${y}] )`); 

        const matrizResultado = nerdamer('C*X+Z').symbol.elements.toString();

        const matrizTemp = matrizResultado.split(',');
        const matrizAux = [Number(eval(matrizTemp[0]).toFixed(9) ), Number( eval(matrizTemp[1]).toFixed(6) ) ];
